            ...
            if config.RECURRENT_UNIT == 'lstm':
                ...
                if config.IS_PRE_IMAGE:
                    feature_pre_images.append(
                        conv3d_init_gru2d_unit(input_pre_images, filter=config.RECURRENT_FILTER, name="feature_pre_images"))
                # deal with pre masks
                feature_pre_masks = []
                if config.IS_PRE_MASK:
                    feature_pre_masks.append(
                        conv3d_init_gru2d_unit(input_pre_masks, filter=config.RECURRENT_FILTER, name="feature_pre_masks"))
                # deal with pre parts
                feature_pre_parts = []
                if config.IS_PRE_PART:
                    feature_pre_parts.append(
                        conv3d_init_gru2d_unit(input_pre_parts, filter=config.RECURRENT_FILTER, name="feature_pre_parts"))
                features = feature_pre_images + feature_pre_masks + feature_pre_parts
                if len(features) > 1:
                    temporal_feature = conv3d_init_gru2d_unit(features,

...
def deeplab_resnet(img_input, architecture):
    ...
    # res1a
    x = KL.Conv2D(16, (3, 3), strides=(1, 1),
                  name='res1a_branch', use_bias=False, padding="same")(img_input)
    x = BatchNorm(axis=-1, name='bn1a_branch', trainable=True)(x)
    x = KL.Activation('relu')(x)
    # res1b
    x = KL.Conv2D(32, (3, 3), strides=(2, 2),
                  name='res1b_branch', use_bias=False, padding="same")(x)
    x = BatchNorm(axis=-1, name='bn1b_branch', trainable=True)(x)
    x = KL.Activation('relu')(x)
    # res1c
    x = KL.ZeroPadding2D((1, 1))(x)
    x = KL.Conv2D(32, (3, 3), strides=(2, 2),
                  name='res1c_branch', use_bias=False)(x)
    x = BatchNorm(axis=-1, name='bn1c_branch', trainable=True)(x)
    C1 = x = KL.Activation('relu')(x)
...
class PARSING_RCNN():
    ...
    def build(self, mode, config):
        ...
        res_model = KM.Model(input_image, c5, name='res_model')
        print(res_model.summary())
        print("res flops:", get_flops(res_model))
        t2 = time()

...
def gru2d_unit(temporal_features, filter, name, kernel_size=(3, 3), padding="same"):
    inputs = []
    for i in temporal_features:
        inputs.append(KL.Conv2D(filter, (3, 3), padding=padding, activation="relu")(i))
    input_tensor = KL.Lambda(lambda x: tf.stack(x, axis=1, name=name + "_stack"))(inputs)
    initial_state = K.mean(input_tensor, axis=1)
    from models.convolutional_recurrent import ConvGRU2D
    x = ConvGRU2D(filters=filter, kernel_size=kernel_size, name=name,
                  padding=padding, return_sequences=False)(input_tensor, initial_state=initial_state)
    x = KL.Activation("relu")(x)
...
def conv3d_unit(temporal_features, filter, name, kernel_size=(3, 3), padding="same"):
    input_tensor = KL.Lambda(lambda x: tf.stack(x, axis=1, name=name + "_stack"))(temporal_features)
    feature_conv3d_1 = KL.Conv3D(filter, (3, 3, 3), padding="same", activation="relu", name=name + "_conv3d1")(
        input_tensor)
    feature_conv3d_2 = KL.Conv3D(filter, (3, 3, 3), padding="same", activation="relu", name=name + "_conv3d2")(
        feature_conv3d_1)
    feature_conv3d_3 = KL.Conv3D(filter, (3, 3, 3), padding="same", activation="relu", name=name + "_conv3d3")(
        feature_conv3d_2)
    x = KL.Lambda(lambda f: K.mean(f, axis=1))(feature_conv3d_3)
    return x
            ...
            if config.RECURRENT_UNIT == 'lstm':
                ...
                if config.IS_PRE_IMAGE:
                    feature_pre_images.append(
                        conv3d_unit(input_pre_images, filter=config.RECURRENT_FILTER,
                                    name="feature_pre_images"))
                # deal with pre masks
                feature_pre_masks = []
                if config.IS_PRE_MASK:
                    feature_pre_masks.append(
                        conv3d_unit(input_pre_masks, filter=config.RECURRENT_FILTER,
                                    name="feature_pre_masks"))
                # deal with pre parts
                feature_pre_parts = []
                if config.IS_PRE_PART:
                    feature_pre_parts.append(
                        conv3d_unit(input_pre_parts, filter=config.RECURRENT_FILTER,
                                    name="feature_pre_parts"))
                features = feature_pre_images + feature_pre_masks + feature_pre_parts
                if len(features) > 1:
                    temporal_feature = conv3d_unit(features,

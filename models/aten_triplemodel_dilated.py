...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    if use_mini_mask:
        mask = util.minimize_mask(bbox, mask, config.MINI_MASK_SHAPE)  # MINI_MASK_SHAPE=(56, 56)
...
def data_generator(dataset, config, shuffle=True, augment=True, random_rois=0,
    ...
    error_count = 0
    # Anchors config.BACKBONE_STRIDES:
    # [anchor_count, (y1, x1, y2, x2)]  shape is (245760=128*128*5*3, 4) min： -271.5290039756342 max:779.5290039756342
    anchors = util.generate_anchors(config.RPN_ANCHOR_SCALES,  # <class 'tuple'>: (32, 64, 128, 256, 384)
                                    ...
                                    # ndarray [[128 128] [ 64  64] [ 32  32] [ 32  32][ 16  16]]
                                    config.BACKBONE_STRIDES[0],  # list [4, 8, 16, 16, 32]
                                    config.RPN_ANCHOR_STRIDE)  # int1
    ...
    while True:
        try:
            ...
            # RPN Targets rpn_match shape: (245760,)  rpn_bbox shape: (256, 4),
...
def global_parsing_encoder_share(feature_map):
    ...
    conv2 = KL.Conv2D(256, (3, 3), padding='same', dilation_rate=(3, 3), activation='relu',
                      name='mrcnn_global_parsing_encoder_c2')
    # x2 = BatchNorm(axis=-1, name='mrcnn_global_parsing_%d_bn1'%num_classes)(x2)

    conv3 = KL.Conv2D(256, (3, 3), padding='same', dilation_rate=(5, 5), activation='relu',
                      name='mrcnn_global_parsing_encoder_c3')
    # x3 = BatchNorm(axis=-1, name='mrcnn_global_parsing_%d_bn2'%num_classes)(x3)

    conv4 = KL.Conv2D(256, (3, 3), padding='same', dilation_rate=(7, 7), activation='relu',

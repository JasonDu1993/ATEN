...
class MFP(object):
    ...
    def build(self, mode, config):
        ...
        mask_inputs = []
        box_inputs = []
        for i in range(num_pre_frame):
            part_inputs.append(KL.Input(shape=config.IMAGE_SHAPE.tolist(), name="part_input" + str(i)))
        for i in range(num_pre_frame):
            mask_inputs.append(KL.Input(shape=config.IMAGE_SHAPE.tolist(), name="mask_input" + str(i)))
        for i in range(num_pre_frame):
            box_inputs.append(KL.Input(shape=[None, 4], name="box_input" + str(i)))
        ...
        # --------feature------------------------------
        final_feature = deeplab_resnet_share()

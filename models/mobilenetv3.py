...
def ConvBnAct(filters, kernel_size=3, stride=1, padding=0, name='CBA', act_layer=keras.layers.ReLU):
    ...
    def forward(x):
        ...
        y = keras.layers.Conv2D(filters=filters, kernel_size=kernel_size, strides=stride, name='{}/conv'.format(name))(
            y)
...
def mobilenetv3(bneck_settings, input_tensor=None, num_classes=1000, width_multiplier=1.0, divisible_by=8,
                model_type='small', include_top=True):
    ...
    return keras.models.Model(inputs=input_tensor, outputs=x), stages
...
def MobileNetV3Small(input_tensor=None, num_classes=1000, width_multiplier=1.0, divisible_by=8, include_top=True):
    ...
    return mobilenetv3(bneck_settings, input_tensor=input_tensor, num_classes=num_classes,

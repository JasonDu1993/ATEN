...
def conv3d_gru2d_unit(temporal_features, filter, name, kernel_size=(3, 3), padding="same"):
    ...
    # stage 1
    feature_conv3d = KL.Conv3D(64, (3, 3, 3), padding="same", activation="relu", name=name + "_conv3ds1a")(
        input_tensor)
    # feature_conv3d = KL.Conv3D(64, (3, 3, 3), padding="same", activation="relu", name=name + "_conv3s1b")(
    #     feature_conv3d)
    # stage 2
    feature_conv3d = KL.MaxPool3D((3, 3, 3), (1, 2, 2), padding="same")(feature_conv3d)
    feature_conv3d = KL.Conv3D(128, (3, 3, 3), padding="same", activation="relu", name=name + "_conv3s2a")(
        feature_conv3d)
    # feature_conv3d = KL.Conv3D(128, (3, 3, 3), padding="same", activation="relu", name=name + "_conv3s2b")(
    #     feature_conv3d)
    # stage 3
    feature_conv3d = KL.MaxPool3D((3, 3, 3), (1, 2, 2), padding="same")(feature_conv3d)
    feature_conv3d = KL.Conv3D(256, (3, 3, 3), padding="same", activation="relu", name=name + "_conv3s3a")(
        feature_conv3d)
    # feature_conv3d = KL.Conv3D(256, (3, 3, 3), padding="same", activation="relu", name=name + "_conv3s3b")(
    #     feature_conv3d)
    # stage 4
    feature_conv3d = KL.Conv3DTranspose(256, (1, 2, 2), strides=(1, 2, 2), activation="relu",
                                        name=name + "_Transposes4a")(feature_conv3d)
    feature_conv3d = KL.Conv3D(128, (3, 3, 3), padding="same", activation="relu", name=name + "_conv3s4a")(
        feature_conv3d)
    # stage 5
    feature_conv3d = KL.Conv3DTranspose(128, (1, 2, 2), strides=(1, 2, 2), activation="relu",
                                        name=name + "_Transposes5a")(feature_conv3d)
    feature_conv3d = KL.Conv3D(filter, (3, 3, 3), padding="same", activation="relu", name=name + "_conv3s5a")(
        feature_conv3d)
...
def global_parsing_encoder(feature_maps):
    ...
    x = KL.Activation('relu')(x)
    x = global_attention_module_f(x, f=512)
    return x


def global_parsing_decoder(feature_map, low_feature_map, temporal_feature=None):
    ...
    multi_feature = [top, low] + temporal
    x = KL.Lambda(lambda x: tf.concat(x, axis=-1))(multi_feature)
...
def global_parsing_graph(feature_map, num_classes):
    x1 = KL.Conv2D(num_classes, (3, 3), padding='same', dilation_rate=(3, 3),
                   name='mrcnn_global_parsing_c1')(feature_map)

    x2 = KL.Conv2D(num_classes, (3, 3), padding='same', dilation_rate=(5, 5),
                   name='mrcnn_global_parsing_c2')(feature_map)

    x3 = KL.Conv2D(num_classes, (3, 3), padding='same', dilation_rate=(7, 7),
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    mask = util.resize_mask(mask, scale, padding, isopencv=False)  # 使用opencv会报错
    part = util.resize_part(part, scale, padding[:2], isopencv=False)  # 这里不建议用opencv
    part_rev = util.resize_part(part_rev, scale, padding[:2], isopencv=False)  # 这里不建议用opencv
    ...
    if use_mini_mask:
        mask = util.minimize_mask(bbox, mask, config.MINI_MASK_SHAPE, isopencv=False)  # 这里不建议用opencv
    ...
    while True:
        try:
            ...
            if config.IS_PRE_IMAGE or config.IS_PRE_MASK or config.IS_PRE_PART:
                pre_image_names = dataset.load_pre_image_names(image_id, key_num=config.PRE_MULTI_FRAMES,
                                                               gap=config.GAP)
            ...
            if b >= batch_size:
                inputs = [batch_images, batch_image_meta,
                          ...
                          batch_gt_parts]
                if config.IS_PRE_IMAGE or config.IS_PRE_MASK or config.IS_PRE_PART:
                    inputs += batch_pre_images + batch_pre_masks + batch_pre_parts
                outputs = []
...
class MFPNet(object):
    ...
    def build(self, mode, config):
        ...
        fine_feature = global_parsing_decoder(coarse_feature, C1, temporal_feature)
    ...
    def unmold_detections(self, detections, mrcnn_mask, mrcnn_global_parsing, image_shape, window, isopencv=False):
        ...
        for i in range(N):
            ...
            full_mask = util.unmold_mask(masks[i], boxes[i], image_shape, isopencv=isopencv)  # use opencv is slow
...
def mold_image(images, config):
    ...
    colors in RGB order.
    """
    return images.astype(np.float32) - config.MEAN_PIXEL_BGR


def unmold_image(normalized_images, config):
    """Takes a image normalized with mold() and returns the original."""
    return (normalized_images + config.MEAN_PIXEL_BGR).astype(np.uint8)

...
from models.mobilenetv3 import mobilenetv3, MobileNetV3Small, MobileNetV3Large
...
class PARSING_RCNN():
    ...
    def build(self, mode, config):
        ...
        _, stages = MobileNetV3Small(input_tensor=input_image, include_top=False)
        t2 = time()
        c1, c2, c3, c4, c5 = stages
        print("deeplab_resnet", t2 - t1, "s")
        coarse_feature = global_parsing_encoder(c5)
        t3 = time()
        print("global_parsing_encoder", t3 - t2, "s")
        fine_feature = global_parsing_decoder(coarse_feature, c2)
        t4 = time()
